...
f = open("Result-1gram/" + ind + "-VERB-Outfile-" + leng + "-grams.csv", "w")
h = open("Result-1gram/" + ind + "-ADJ-Outfile-" + leng + "-grams.csv", "w")
w = csv.writer(f)
x = csv.writer(h)
...
df = pd.read_csv("Result-1gram/" + ind + "-VERB-Outfile-" + leng + "-grams.csv", header = 0, index_col = ["Ngrams"])
dfnew = df.groupby(df.index).sum()
dfnew.to_csv("Result-1gram/" + ind + "-VERB-Result-" + leng + "-grams.csv")

sorted_list = {}
list1 = []
list2 = []

with open("Result-1gram/" + ind + "-VERB-Result-" + leng + "-grams.csv", 'r+') as inf:
    for line in inf:
        ...
        for i in reader:
            ...
            try:
                ...
                print("Don't have past past_participle")
                sorted_list[i[0]] = 0
                past_part = '-'
...
with open("Result-1gram/" + ind + "-VERB-Result-" + leng + "-grams.csv", 'w') as csv_file:
    writer = csv.writer(csv_file)
    writer.writerow(["VERB", "PP_ADJ_Count"])
    for key, value in sorted(sorted_list.items()):
        writer.writerow([key, value])
csv_file.close()

df = pd.read_csv("Result-1gram/" + ind + "-VERB-Result-" + leng + "-grams.csv", header = 0, index_col = ["VERB"])
df['Past_Participle'] = list1
df['VERB_COUNT'] = list2
df.to_csv("Result-1gram/" + ind + "-VERB-Result-" + leng + "-grams.csv")
...
h.close()
df = pd.read_csv("Result-1gram/" + ind + "-ADJ-Outfile-" + leng + "-grams.csv", header=0, index_col=["Ngrams"])
dfnew = df.groupby(df.index).sum()
dfnew.to_csv("Result-1gram/" + ind + "-ADJ-Result-" + leng + "-grams.csv")
