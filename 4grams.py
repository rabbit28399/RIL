...
x = open("Result-4gram/"+index + "-Outfile-" + ngramLen + "gram-verb-adj.csv", "w")
w = csv.writer(x)
...
df = pd.read_csv("Result-4gram/"+index + "-Outfile-" + ngramLen + "gram-verb-adj.csv", header = 0, index_col = ["Verb : Adj : Pattern"])
dfnew = df.groupby(df.index).sum()
dfnew.to_csv("Result-4gram/"+index + "-Outfile-" + ngramLen + "gram-verb-adj.csv")

pat_newd = {}
patd = {}
with open("Result-4gram/"+index + "-Outfile-" + ngramLen + "gram-verb-adj.csv", 'r+') as inf:
    for line in inf:
        ...
        for i in reader:
            ...
            if pat_new2 and pat_new3:
                if pat[0] + ":" + pat[1] in patd:
                    if pat[0] + ":" + pat[1] + ":" + pat_new2 + "-" + pat_new3  in pat_newd:
                        pat_newd[pat[0] + ":" + pat[1] + ":" + pat_new2 + "-" + pat_new3] += int(i[1])
                    else:
                        pat_newd[pat[0] + ":" + pat[1] + ":" + pat_new2 + "-" + pat_new3] = int(i[1])
                else:
...
with open("Result-4gram/"+index + "-Result-" + ngramLen + "gram-verb-adj.csv", 'w') as csv_file:
